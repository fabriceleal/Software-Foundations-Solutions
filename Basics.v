...
Proof.
  intros b c.
  intros H.
  assert (HA: andb true c = c).
  reflexivity.
  assert (HO: orb false c = c).
  reflexivity.
  destruct b.
  rewrite <- HA.
  rewrite -> H.
...
Proof. reflexivity. Qed.
Example test_bin_incr2: bin_to_nat(D (incr Z)) = 2.
Proof. reflexivity. Qed.
Example test_bin_incr3: bin_to_nat(F (incr Z)) = 3.
Proof. reflexivity. Qed.
Example test_bin_incr4: bin_to_nat(D (D (F Z))) = 4.
Proof. reflexivity. Qed.
Example test_bin_incr5: bin_to_nat(F (incr (incr Z))) = 5.
Proof. reflexivity. Qed.
(** [] *)
...
(* Fixpoint plus_or_half (b: bin) : bin := *)
(*   match b with *)
(*     | Z => Z *)
(*     | D b' => plus_or_half b' *)
(*     | F b' => plus_or_half (D (incr b')) *)
(*   end. *)
(** [] *)
