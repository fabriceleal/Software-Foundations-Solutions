...
Proof.
  apply nat_ind.
  reflexivity.
  intros. rewrite <- H. rewrite plus_comm.
  reflexivity.
Qed.
(** [] *)
...
Definition P_plus_comm_n : nat -> Prop :=
  fun n => forall m, n + m = m + n.

Theorem plus_comm_explicit: forall n : nat, P_plus_comm_n n.
Proof.
  intros. induction n.
  unfold P_plus_comm_n. intros. rewrite <- plus_n_O. reflexivity.
  unfold P_plus_comm_n in IHn. unfold P_plus_comm_n.
  intros. simpl. rewrite <- plus_n_Sm. apply f_equal. apply IHn.
Qed.

Definition P_plus_assoc_n_m : nat -> nat -> Prop :=
  fun n => fun m => forall p, n + (m + p) = n + m + p.

Theorem plus_assoc_explicit: forall n m: nat, P_plus_assoc_n_m n m.
Proof.
  intros. unfold P_plus_assoc_n_m.
